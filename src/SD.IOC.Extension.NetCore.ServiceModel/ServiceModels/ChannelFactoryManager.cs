...
namespace System.ServiceModel.Extensions
{
    ...
    internal sealed class ChannelFactoryManager : IDisposable
    {
        ...
        private Binding GetBinding<T>()
        {
            ...
            if (!Constants.AvailableBindings.Contains(endpoint.Binding))
            {
            ...
            if (endpoint.Binding == Constants.BasicHttpBinding)
            {
                if (string.IsNullOrWhiteSpace(endpoint.BindingConfiguration))
                {
                    return new BasicHttpBinding();
                }
                if (!ServiceModelSetting.BasicHttpBindings.TryGetValue(endpoint.BindingConfiguration, out BasicHttpBindingElement basicHttpBindingElement))
                {
            ...
            if (endpoint.Binding == Constants.NetTcpBinding)
            {
                if (string.IsNullOrWhiteSpace(endpoint.BindingConfiguration))
                {
                    return new NetTcpBinding();
                }
                if (!ServiceModelSetting.NetTcpBindings.TryGetValue(endpoint.BindingConfiguration, out NetTcpBindingElement netTcpBindingElement))
                {
                    throw new KeyNotFoundException($"未找到名称为\"{endpoint.BindingConfiguration}\"的绑定配置！");
                }
            ...
            if (endpoint.Binding == Constants.NetHttpBinding)
            {
                if (string.IsNullOrWhiteSpace(endpoint.BindingConfiguration))
                {
                    return new NetHttpBinding();
                }
                if (!ServiceModelSetting.NetHttpBindings.TryGetValue(endpoint.BindingConfiguration, out NetHttpBindingElement netHttpBindingElement))
                {
                    throw new KeyNotFoundException($"未找到名称为\"{endpoint.BindingConfiguration}\"的绑定配置！");
                }
            ...
            if (endpoint.Binding == Constants.WSHttpBinding)
            {
                if (string.IsNullOrWhiteSpace(endpoint.BindingConfiguration))
                {
                    return new WSHttpBinding();
                }
                if (!ServiceModelSetting.WSHttpBindings.TryGetValue(endpoint.BindingConfiguration, out WSHttpBindingElement wsHttpBindingElement))
                {
                    throw new KeyNotFoundException($"未找到名称为\"{endpoint.BindingConfiguration}\"的绑定配置！");
                }
